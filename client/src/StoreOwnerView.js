...
class StoreOwnerView extends Component {
  ...
  fetchData() {
    ...
    storeContract.at(this.props.address)
      ...
      .then(store => Promise.all([
        store.MEWname(),
        store.MEWdescription(),
        store.MEWnewestProductSku(),
        store.MEWstopped()
      ]))
      // save store metadata
      .then(data => {
      ...
      .then(newestProductSku => {
        ...
        for (let i = 1; i <= newestProductSku; i++) {
          result.push(instance.MEWproductsBySku(i));
        }
  ...
  addNewProduct() {
    ...
    this.setState({loadingNewProduct: true}, () => {
      ...
      .then(() => {
        var event = this.state.instance.MEWNewProductAdded(
        ...
        event.watch((error, result) => {
          if (!error) {
            this.state.instance.MEWnewestProductSku()
              .then(res => res.toNumber())
              .then(latestSku => {
                const result = [];
                for (let i = 1; i <= latestSku; i++) {
                  result.push(this.state.instance.MEWproductsBySku(i));
                }
                return Promise.all(result);
              })
              .then(data => {
  ...
  sendUpdateInventoryCount(sku, newCount) {
    this.state.instance.MEWupdateInventoryCount.sendTransaction(
      sku,
      newCount,
      {from: this.props.currentAccount});
  }
  ...
  toggleEmergencyStop() {
    this.setState({loadingStoppedStateChange: true}, () => {
      this.state.instance.MEWtoggleContractActive.sendTransaction({from: this.props.currentAccount})
        .then(() => {
