...
class ShopperComponent extends Component {
  ...
  componentDidMount() {
    ...
    marketplace.MEWNewStoreCreated({}, { fromBlock: 0, toBlock: 'latest' }).get((error, result) => {
      if (!error) {
  ...
  handleRequestStoreOwnership() {
    this.setState(
      ...
      () => this.props.marketplace.MEWaddStoreOwnerRequest({from: this.props.currentAccount}));


    this.props.marketplace.MEWStoreOwnerRequestSent().watch((error, result) => {
      if (!error) {
